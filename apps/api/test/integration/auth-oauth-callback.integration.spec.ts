...
describe('OAuth Callback Integration', () => {
  ...
  describe('GET /auth/strava/callback', () => {
    it('should complete OAuth flow and set cookies on success', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/auth/strava/callback?code=test-code');
    ...
    it('should redirect to onboarding for new user', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/auth/strava/callback?code=test-code-new-user');
    ...
    it('should redirect to dashboard for returning user', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/auth/strava/callback?code=test-code-returning');
    ...
    it('should redirect to error page when Strava returns error', async () => {
      const response = await request(app.getHttpServer()).get('/auth/strava/callback?error=access_denied&scope=read');
    ...
    it('should redirect to error page when code missing', async () => {
      const response = await request(app.getHttpServer()).get('/auth/strava/callback');
    ...
    it('should redirect to error page on Strava API failure', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/auth/strava/callback?code=invalid-code');
    ...
    it('should update existing user profile on re-authentication', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/auth/strava/callback?code=test-code-update');
    ...
    it('should create new refresh token on each login', async () => {
      ...
      await request(app.getHttpServer()).get('/auth/strava/callback?code=first-login');
      ...
      await request(app.getHttpServer()).get('/auth/strava/callback?code=second-login');
